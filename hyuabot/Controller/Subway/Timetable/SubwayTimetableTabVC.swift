...
class SubwayTimetableTabVC: UIViewController {
    ...
    private lazy var subwayTimetableTableView: UITableView = .init().then {
        $0.delegate = self
        $0.dataSource = self
    ...
    private func setupUI() {
        view.addSubview(subwayTimetableTableView)
        subwayTimetableTableView.snp.makeConstraints { make in
            make.edges.equalToSuperview()
        }
    }

    func reload() {
        subwayTimetableTableView.reloadData()
    }
...
extension SubwayTimetableTabVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        guard let cell = tableView.dequeueReusableCell(withIdentifier: SubwayTimetableCellView.reuseIdentifier) as? SubwayTimetableCellView
        else { return SubwayTimetableCellView() }
        if isWeekdays {
