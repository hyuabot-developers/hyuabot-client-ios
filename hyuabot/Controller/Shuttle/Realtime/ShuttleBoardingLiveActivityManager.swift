...
import UIKit
    ...
    final class ShuttleBoardingLiveActivityManager: NSObject, @preconcurrency CLLocationManagerDelegate {
        ...
        @available(iOS 16.1, *)
        private func scheduleAlightingUpdates(
            for context: ShuttleAlarmContext,
            destination: ShuttleAlarmStop,
            ...
            updateTasks[context.key] = Task { [weak self] in
                ...
                while !Task.isCancelled {
                    let now = Date.now
                    if now >= destination.time.addingTimeInterval(alightingGraceInterval) {
                        await endActivity(activity)
                        await MainActor.run {
                            self.updateTasks[context.key] = nil
                    ...
                    if let distance = distanceFromCurrentLocation(to: destination),
                       distance <= alightingAlertRadius
                    {
                        ShuttleAlarmNotificationService.shared.fireAlightingProximityAlert(
                            context: context,
        ...
        private func endActivity(_ activity: Activity<ShuttleBoardingActivityAttributes>) async {
            if #available(iOS 16.2, *) {
                let state = ShuttleBoardingActivityAttributes.ContentState(
                    titleText: activity.attributes
                        .alarmKind == "alighting" ? String(localized: "shuttle.alarm.alighting.live.title") :
                        String(localized: "shuttle.alarm.boarding"),
                    statusText: activity.attributes
                        .alarmKind == "alighting" ? String(localized: "shuttle.alarm.alighting.live.completed") :
                        String(localized: "shuttle.alarm.boarding.live.completed"),
                    dynamicIslandStatusText: activity.attributes
                        .alarmKind == "alighting" ? String(localized: "shuttle.alarm.alighting.live.completed") :
                        String(localized: "shuttle.alarm.boarding.live.completed"),
                    currentStopName: activity.attributes
                        .alarmKind == "alighting" ? (activity.attributes.targetStopName ?? activity.attributes.boardingStopName) : activity
                        .attributes.boardingStopName,
                    nextStopName: "",
                    checkpointStopNames: [activity.attributes
                        .alarmKind == "alighting" ? (activity.attributes.targetStopName ?? activity.attributes.boardingStopName) : activity
                        .attributes.boardingStopName],
                    checkpointTimes: [],
                    checkpointWaitingFormat: String(localized: "shuttle.alarm.checkpoint.waiting"),
        ...
        @available(iOS 16.1, *)
        private func contentState(for context: ShuttleAlarmContext, destination: ShuttleAlarmStop) -> ShuttleBoardingActivityAttributes
            .ContentState
        {
            let checkpointStops = alightingCheckpointStops(context: context, destination: destination)
            let checkpointTimes = checkpointStops.map(\.time)
            ...
            return ShuttleBoardingActivityAttributes.ContentState(
                ...
                checkpointApproachingFormat: String(localized: "shuttle.alarm.checkpoint.approaching"),
                checkpointDepartedFormat: String(localized: "shuttle.alarm.checkpoint.departed"),
                progress: checkpointTimes
                    .count >= 2 ? checkpointProgress(checkpointTimes) :
                    alightingProgress(distance: distanceFromCurrentLocation(to: destination)),
                progressSegments: checkpointProgressSegments(checkpointTimes)
            )
        }
        ...
        private func alightingProgress(distance: CLLocationDistance?) -> Int {
            ...
            let cappedDistance = min(Int(distance), 2000)
                ...
                for activity in Activity<ShuttleBoardingActivityAttributes>.activities where activity.attributes.alarmKind == "alighting" {
                    ...
                    if #available(iOS 16.2, *) {
                        let content = ActivityContent(
                            state: state,
                            staleDate: value.destination.time.addingTimeInterval(self.alightingGraceInterval)
                        )
                        await activity.update(content)
                    } else {
                        await activity.update(using: state)
